...
<svelte:head>
	<title>Sign In | Facet-Demo</title>
</svelte:head>

<div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
	<div class="card p-8 max-w-md w-full">
		<div class="text-center mb-8">
			<h1 class="text-2xl font-bold text-gray-900 dark:text-white">Sign in to Facet-Demo</h1>
			<p class="text-gray-600 dark:text-gray-400 mt-2">Public demo - data resets daily</p>
		</div>
		...
		<div class="mb-6 p-4 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
			<div class="flex items-start gap-3">
				...
				<div class="text-sm">
					<p class="font-medium text-amber-800 dark:text-amber-200">Demo Credentials</p>
					<p class="text-amber-700 dark:text-amber-300 mt-1">
						Email: <code class="bg-amber-100 dark:bg-amber-900/40 px-1 rounded">demo@example.com</code><br/>
						Password: <code class="bg-amber-100 dark:bg-amber-900/40 px-1 rounded">demo123</code>
					</p>
				</div>
			</div>
		</div>
		...
		{:else}
			<div class="mb-4 text-sm text-gray-600 dark:text-gray-400">
				OAuth login isn't configured. Use password login below.
			</div>
		{/if}
